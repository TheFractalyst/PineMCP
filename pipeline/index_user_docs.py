...
def discover_extra_docs() -> list[Path]:
    """Find extra .txt documentation files from data/user_docs/."""
    files: list[Path] = []
    for name in EXTRA_DOCS_FILES:
        f = EXTRA_USER_DOCS_DIR / name
        if f.is_file() and f.stat().st_size > SKIP_THRESHOLD:
            files.append(f)
        else:
            logger.warning(f"Extra doc not found: {name}")
    return files
...
def main():
    ...
    # Also discover extra .txt docs from data/user_docs/
    extra_files = discover_extra_docs()
    if extra_files:
        logger.info(f"Found {len(extra_files)} extra optimization docs")
    ...
    # Process extra .txt docs (optimization-relevant)
    for f in extra_files:
        try:
            entries = process_file(f, f.parent)
            all_entries.extend(entries)
            file_stats.append((f.name, len(entries)))
            if entries:
                logger.info(f"  {f.name:<55s} {len(entries):>4d} chunks (extra)")
        except Exception as e:
            logger.error(f"  Error processing {f}: {e}")
