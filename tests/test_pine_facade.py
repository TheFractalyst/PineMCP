...
class TestCallPineFacadeCircuitBreaker:
    ...
    @pytest.mark.asyncio
    async def test_falls_back_to_syntax_check_when_circuit_open(self):
        """Should fall back to local syntax check when circuit breaker is open."""
        pine_cb.open_until = time.time() + 60  # Force open

        result = await call_pine_facade("//@version=6\nindicator(\"test\")")

        assert result["meta"].get("fallback") == "syntax_check"
        assert result["success"] is True  # Valid code passes syntax check

    @pytest.mark.asyncio
    async def test_syntax_check_catches_errors_when_circuit_open(self):
        """Syntax check fallback should catch PineScript errors."""
        pine_cb.open_until = time.time() + 60  # Force open

        result = await call_pine_facade("//@version=5\nstudy(\"test\")\nplot(close)")
...
class TestCallPineFacadeHTTPStatusHandling:
    ...
    @pytest.mark.asyncio
    @patch("core.pine_facade.PINE_FACADE_FALLBACK_ENABLED", False)
    @patch("core.pine_facade.get_facade_client")
    async def test_http_403_returns_error(self, mock_get_client):
        """HTTP 403 should return user-friendly error when fallback disabled."""
    ...
    @pytest.mark.asyncio
    @patch("core.pine_facade.get_facade_client")
    async def test_http_403_falls_back_to_syntax_check(self, mock_get_client):
        """HTTP 403 should fall back to syntax check when fallback enabled."""
        mock_response = MagicMock()
        mock_response.status_code = 403
        mock_response.text = "Access denied"
        mock_response.headers = {}
    ...
    async def test_http_503_returns_error(self, mock_get_client):
        """HTTP 503 should return service unavailable error when fallback disabled."""
        mock_response = MagicMock()
    ...
    async def test_http_502_504_handled(self, mock_get_client):
        """HTTP 502/504 should be handled like 503 when fallback disabled."""
...
class TestCallPineFacadeNetworkErrors:
    ...
    async def test_connect_error_returns_user_friendly(self, mock_get_client):
        """ConnectError should return user-friendly message when fallback disabled."""
        mock_client = AsyncMock()
    ...
    @pytest.mark.asyncio
    @patch("core.pine_facade.get_facade_client")
    async def test_connect_error_falls_back_to_syntax_check(self, mock_get_client):
        """ConnectError should fall back to syntax check when fallback enabled."""
        mock_client = AsyncMock()
        mock_client.post.side_effect = httpx.ConnectError("Connection refused")
        mock_get_client.return_value = mock_client
    ...
    async def test_timeout_error_returns_user_friendly(self, mock_get_client):
        """Timeout should return user-friendly message when fallback disabled."""
        mock_client = AsyncMock()
    ...
    async def test_oserror_returns_user_friendly(self, mock_get_client):
        """OSError should return user-friendly message when fallback disabled."""
        mock_client = AsyncMock()
...
# -----------------------------------------------------------------------------
# local_syntax_check Tests
# -----------------------------------------------------------------------------


class TestLocalSyntaxCheck:
    """Test the local syntax validator (fallback when facade is down)."""

    def test_valid_code_passes(self):
        """Valid v6 code should pass syntax check."""
        result = local_syntax_check('//@version=6\nindicator("test")\nplot(close)')
        assert result["success"] is True
        assert result["errors"] == []
        assert result["meta"]["fallback"] == "syntax_check"

    def test_missing_version_header(self):
        """Missing //@version=6 should be flagged."""
        result = local_syntax_check('indicator("test")\nplot(close)')
        assert result["success"] is False
        assert any("version=6" in e["text"].lower() for e in result["errors"])

    def test_wrong_version(self):
        """Wrong version should be flagged."""
        result = local_syntax_check('//@version=5\nstudy("test")\nplot(close)')
        assert result["success"] is False
        assert any("version 5" in e["text"].lower() for e in result["errors"])

    def test_study_instead_of_indicator(self):
        """study() should be flagged as v5 syntax."""
        result = local_syntax_check('//@version=6\nstudy("test")\nplot(close)')
        assert result["success"] is False
        assert any("study()" in e["text"].lower() for e in result["errors"])

    def test_bare_v4_color_constant(self):
        """Bare color names should be flagged."""
        result = local_syntax_check('//@version=6\nindicator("test")\nplot(close, color=red)')
        assert result["success"] is False
        assert any("color.red" in e["text"] for e in result["errors"])

    def test_unclosed_parenthesis(self):
        """Unclosed parenthesis should be flagged."""
        result = local_syntax_check('//@version=6\nindicator("test"\nplot(close)')
        assert result["success"] is False
        assert any("unclosed" in e["text"].lower() for e in result["errors"])

    def test_reassign_before_declare(self):
        """:= before = should be flagged."""
        result = local_syntax_check('//@version=6\nindicator("test")\nx := 5\nplot(x)')
        assert result["success"] is False
        assert any(":=" in e["text"] for e in result["errors"])

    def test_na_comparison_with_equals(self):
        """x == na should be flagged."""
        result = local_syntax_check('//@version=6\nindicator("test")\nif close == na\n    plot(close)')
        assert result["success"] is False
        assert any("na(x)" in e["text"] for e in result["errors"])

    def test_na_comparison_with_not_equals(self):
        """x != na should be flagged."""
        result = local_syntax_check('//@version=6\nindicator("test")\nif close != na\n    plot(close)')
        assert result["success"] is False
        assert any("not na(x)" in e["text"] for e in result["errors"])

    def test_strategy_call_without_declaration(self):
        """strategy.* without strategy() should be flagged."""
        result = local_syntax_check('//@version=6\nindicator("test")\nstrategy.entry("Long", true)')
        assert result["success"] is False
        assert any("strategy()" in e["text"] for e in result["errors"])

    def test_bare_ta_function(self):
        """Bare TA function names should be flagged."""
        result = local_syntax_check('//@version=6\nindicator("test")\nplot(ema(close, 14))')
        assert result["success"] is False
        assert any("ta.ema" in e["text"] for e in result["errors"])

    def test_transp_parameter_warning(self):
        """transp= should produce a warning."""
        result = local_syntax_check('//@version=6\nindicator("test")\nplot(close, color=color.red, transp=50)')
        assert any("transp" in w["text"].lower() for w in result["warnings"])

    def test_array_get_warning(self):
        """array.get() should produce a bounds-check warning."""
        result = local_syntax_check('//@version=6\nindicator("test")\narr = array.from(1, 2, 3)\nplot(array.get(arr, 0))')
        assert any("array.get" in w["text"].lower() for w in result["warnings"])

    def test_varip_misuse_warning(self):
        """varip with non-input value should produce a warning."""
        result = local_syntax_check('//@version=6\nindicator("test")\nvarip x = close * 2\nplot(x)')
        assert any("varip" in w["text"].lower() for w in result["warnings"])

    def test_proper_code_no_false_positives(self):
        """Properly written v6 code should not produce false positives."""
        code = (
            '//@version=6\n'
            'indicator("My Indicator", overlay=true)\n'
            'src = input(close, "Source")\n'
            'len = input.int(14, "Length")\n'
            'myEma = ta.ema(src, len)\n'
            'plot(myEma, color=color.blue)\n'
        )
        result = local_syntax_check(code)
        assert result["success"] is True
        assert result["errors"] == []

    def test_string_literals_not_analyzed(self):
        """String literals should not be analyzed for syntax errors."""
        code = (
            '//@version=6\n'
            'indicator("test(unclosed")\n'
            'plot(close, color=color.red)\n'
        )
        result = local_syntax_check(code)
        assert result["success"] is True  # The ( in string should not break paren count

    def test_comments_not_analyzed(self):
        """Comments should not be analyzed for syntax errors."""
        code = (
            '//@version=6\n'
            'indicator("test")\n'
            '// plot(close, color=red) - this is a comment\n'
            'plot(close, color=color.red)\n'
        )
        result = local_syntax_check(code)
        assert result["success"] is True  # The red in comment should not trigger error

    def test_returns_correct_format(self):
        """Result should match call_pine_facade format."""
        result = local_syntax_check("//@version=6\nindicator(\"test\")\nplot(close)")
