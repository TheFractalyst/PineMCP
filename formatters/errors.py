...
_FIX_HINTS: dict[str, str] = {
    ...
    "repainting": "Signal uses future data or unconfirmed bar values. Guard with barstate.isconfirmed. For non-repainting HTF data: use close[1] with lookahead=barmerge.lookahead_on (PineCoders recommended pattern). The [1] offset and lookahead_on are interdependent - neither can be removed.",
    "request.security_lower_tf": "request.security_lower_tf() returns an array of intrabar values from a lower timeframe. Max 100K intrabars. Use timeframe.in_seconds() to calculate appropriate LTF. Prefer request.security_lower_tf() over request.security() for LTF data.",
    "intrabar": "Intrabars are bars from a timeframe lower than the chart's. request.security_lower_tf() returns arrays of intrabar values. Cannot identify first intrabar on realtime bars - results differ between historical and realtime. Do NOT use intrabar first-bar detection for alerts or orders.",
    "100K intrabar": "request.security_lower_tf() has a 100K intrabar limit. Lower LTF = more intrabars per chart bar = fewer chart bars covered. Trade-off: precision vs coverage. Use array.size() to check actual intrabar count.",
    "timeframe.in_seconds": "v6 timeframe.in_seconds() returns chart/current timeframe in seconds. Use to compare timeframes, calculate multiples, or validate HTF/LTF relationships: `if timeframe.in_seconds() > timeframe.in_seconds(htfInput)`.",
